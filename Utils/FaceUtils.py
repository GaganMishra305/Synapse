...
def predict_emotion_rf(frame, face_landmarks):
    ...
    cv2.putText(frame, emotions[int(output[0])],(10, frame.shape[0] - 10),cv2.FONT_HERSHEY_SIMPLEX,2,(0, 0, 0),3)
...
def predict_emotion_cnn(frame, faces):
    ...
    for (x, y, w, h) in faces:
        ...
        prediction = emotion_detector_cnn.predict(cropped_img, verbose=0)
